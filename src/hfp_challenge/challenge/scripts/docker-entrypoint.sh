...
echo "[INFO]: Running '${HFP_CHALLENGE_API_SLUG}' docker-entrypoint.sh..."
...
main()
{
	...
	find "${HFP_CHALLENGE_HOME_DIR}" \
		"${HFP_CHALLENGE_API_CONFIGS_DIR}" \
		"${HFP_CHALLENGE_API_DATA_DIR}" \
		"${HFP_CHALLENGE_API_LOGS_DIR}" \
		"${HFP_CHALLENGE_API_TMP_DIR}" \
		\( \
			-type d -name ".git" -o \
			-type d -name ".venv" -o \
			-type d -name "venv" -o \
	...
	find "${HFP_CHALLENGE_API_DIR}" "${HFP_CHALLENGE_API_CONFIGS_DIR}" "${HFP_CHALLENGE_API_DATA_DIR}" \
		\( \
			-type d -name ".git" -o \
			-type d -name ".venv" -o \
			-type d -name "venv" -o \
	...
	find "${HFP_CHALLENGE_API_LOGS_DIR}" "${HFP_CHALLENGE_API_TMP_DIR}" -type d -exec chmod 775 {} + || exit 2
	find "${HFP_CHALLENGE_API_LOGS_DIR}" "${HFP_CHALLENGE_API_TMP_DIR}" -type f -exec chmod 664 {} + || exit 2
	find "${HFP_CHALLENGE_API_LOGS_DIR}" "${HFP_CHALLENGE_API_TMP_DIR}" -type d -exec chmod +s {} + || exit 2
